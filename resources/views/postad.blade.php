...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gradient-to-r from-indigo-500 via-pink-500 to-yellow-500">
        <div class="w-full min-h-screen bg-white bg-opacity-20 backdrop-filter backdrop-blur-md">
            ...
            <div class="bg-cover min-h-screen w-full flex justify-center items-center">
                @include('components.filter')

                <div class="w-3/12 mx-auto">
                    <div class="flex flex-row space-y-2 justify-center h-full py-4 border-2 border-white rounded-xl space-x-3">
                        <div class="flex flex-col space-y-2 w-auto">
                            <img class="flex-1 h-5/6 rounded-lg" src="{{url('/images/exRaidIcon.png')}}" />
                            <div class="opacity-95 border rounded-lg border-white px-4 hover:bg-white hover:bg-opacity-20">
                                <p class="m-auto inset-0 text-sm font-medium leading-normal text-center text-white py-2">contact</p>
                            </div>
                        </div>
                        <div class="w-3/4">
